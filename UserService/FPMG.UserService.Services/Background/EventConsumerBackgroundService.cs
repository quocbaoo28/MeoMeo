using FPMG.UserService.Services.Messaging;
...
namespace FPMG.UserService.Services.Background
{
    public class EventConsumerBackgroundService : BackgroundService
    {
        ...
        private async Task DummyExamServiceConsumer()
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                await eventConsumer.ConsumeEventAsync<List<Dictionary<string, string>>>("user_serrvice", async stockList =>
                {
                });
            }
        }
