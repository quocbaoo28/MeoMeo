...
namespace FPMG.GradingService.Services.Background
{
    public class EventConsumerBackgroundService : BackgroundService
    {
        ...
        {
            Task.Run(async () => await DummyGradingServiceConsumer()),
        ...
        private async Task DummyGradingServiceConsumer()
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                var eventConsumer = scope.ServiceProvider.GetRequiredService<IEventConsumer>();

                await eventConsumer.ConsumeEventAsync<List<Dictionary<string, string>>>("grading_service", async stockList =>
                {
                    Console.WriteLine("Received event grading service");
                });
            }
        }
